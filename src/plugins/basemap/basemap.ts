...
import ExtLayer from '../../extensions/layer.extension'
...
export class Basemap extends WebMapPlugin<{
  ...
  /** 底图项容器图层组 */
  private _layerGroup: ExtLayer<LayerGroup>

  /** 底图项容器池 */
  private _basemapItemPool : Map<string, TileLayer[]> = new Map()
  ...
  /** 初始化 */
  private _init () : this {
    this._layerGroup = new ExtLayer(new LayerGroup())
      .addTo(this.map_)
  ...
  private _createTianDiTu () : this {
    const createTianDiTuItem = (name: string, proj: string) => {
      this.createBasemapItem(`天地图${name}${proj}`, new TileLayer(Basemap[`_TianDiTu${proj}Urls`][`${name}底图`]))
      this.createBasemapItem(`天地图${name}含注记${proj}`, [
        new TileLayer(Basemap[`_TianDiTu${proj}Urls`][`${name}底图`]),
        new TileLayer(Basemap[`_TianDiTu${proj}Urls`][`${name}注记`]),
      ])
  ...
  private _createGeoQDiTu () : this {
    Object.entries(Basemap._GeoQUrls).forEach(
      ([key, url]) => this.createBasemapItem(key, new TileLayer(url))
    )
    return this
  }
  ...
  public reSortLayers () : this {
    this._layerGroup.target.setZIndex(0)
    return this
  }
  ...
  public setBasemap (key: string) : this {
    if (this._basemapItemPool.has(key)) {
      ...
      this._layerGroup.target.clearLayers()
      layers.forEach(lyr => this._layerGroup.target.addLayer(lyr))
      this._key = key
   ...
   * @param layer 底图图层
   */
  public createBasemapItem (key: string, layer: TileLayer) : this
  /**
   * 创建底图项
   * @param key 底图项Key值
   * @param layers 底图图层数组
   */
  public createBasemapItem (key: string, layers: TileLayer[]) : this
  /**
   * 创建底图项
   * @param key 底图项
   ...
   * @returns this
   */
  public createBasemapItem (key: string, arg1: TileLayer | TileLayer[]) : this {
    const layers = Array.isArray(arg1) ? arg1 : [arg1]
    this._basemapItemPool.set(key, layers)
    return this
  }
   ...
   * @param layer 底图图层
   */
  public createBasemapItemAndSet (key: string, layer: TileLayer) : this
  /**
   * 创建并设置底图项
   * @param key 底图项Key值
   * @param layers 底图图层数组
   */
  public createBasemapItemAndSet (key: string, layers: TileLayer[]) : this
  /**
   * 创建并设置底图项
   * @param key 底图项
   * @param arg1 底图图层 or 底图图层数组
   * @returns this
   */
  public createBasemapItemAndSet (key: string, arg1: TileLayer | TileLayer[]) : this {
